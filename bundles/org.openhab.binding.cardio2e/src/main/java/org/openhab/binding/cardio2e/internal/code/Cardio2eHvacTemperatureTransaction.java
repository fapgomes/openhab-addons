        ...
        if (isDataVerified()) {
            switch (getTransactionType()) {
                ...
                case NACK:
                    ...
                    break;
                case INFORMATION:
                    returnString = String.format("%s%s %s %d %.2f %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(), hvacTemperature,
                            hvacSystemMode.symbol, CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
            }
        }
        return returnString;
