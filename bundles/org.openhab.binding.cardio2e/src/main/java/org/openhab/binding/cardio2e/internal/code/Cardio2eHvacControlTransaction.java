...
public class Cardio2eHvacControlTransaction extends Cardio2eTransaction {
    ...
    public Cardio2eHvacControlTransaction(short objectNumber, double hvacHeatingSetPoint, double hvacCoolingSetPoint,
            Cardio2eHvacFanStates hvacFanState, Cardio2eHvacSystemModes hvacSystemMode) { // Simple HVAC control SET
                                                                                          // constructor.
                    ...
                    default:
                        throw new IllegalArgumentException(
                                "Cannot determine which setpoint option (heating or cooling) is suitable for '"
                                        + singleHvacSystemMode + "' simpleControlHvacSystemMode");
                }
            }
        } else {
            throw new IllegalArgumentException("To use getSingleHvacSetPoint(), singleHvacSystemMode cannot be null");
        }
    ...
    public int getKnxDpt20_105() {
        ...
        if (hvacSystemMode != null) {
            switch (hvacSystemMode) {
                ...
                default:
                    throw new IllegalArgumentException(
                            "Cannot parse '" + hvacSystemMode + "' Cardio 2é HVAC System Mode to KNX DPT 20.105 value");
            }
        } else {
            throw new IllegalArgumentException("Cannot parse null hvacSystemMode to KNX DPT 20.105");
        }
    ...
    public void setKnxDpt20_105(int dpt20_105) {
        switch (dpt20_105) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Cannot parse KNX DPT 20.105 '" + dpt20_105 + "' value to a Cardio 2é HVAC System Mode");
        }
    }
    ...
    public boolean isDataVerified() { // Returns true if object data matches
        ...
        if (super.isDataVerified()) {
            switch (getTransactionType()) {
                ...
                case INFORMATION:
                case SET:
                    verified = ((getObjectNumber() != -1) && (hvacHeatingSetPoint != -1.00)
                            && (hvacCoolingSetPoint != -1.00) && (hvacFanState != null) && (hvacSystemMode != null));
                    break;
            }
        }
        return verified;
        ...
        if (isDataVerified()) {
            switch (getTransactionType()) {
                ...
                case NACK:
                    ...
                    break;
                case INFORMATION:
                    returnString = String.format("%s%s %s %d %.2f %.2f %s %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(), hvacHeatingSetPoint,
                            hvacCoolingSetPoint, hvacFanState.symbol, hvacSystemMode.symbol,
                            CARDIO2E_END_TRANSACTION_CHARACTER);
                case SET:
                    returnString = String.format("%s%s %s %d %.0f %.0f %s %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(), hvacHeatingSetPoint,
                            hvacCoolingSetPoint, hvacFanState.symbol, hvacSystemMode.symbol,
                            CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
