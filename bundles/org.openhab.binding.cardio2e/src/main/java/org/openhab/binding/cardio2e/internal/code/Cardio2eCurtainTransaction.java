...
public class Cardio2eCurtainTransaction extends Cardio2eTransaction {
    ...
    public void setOpeningPercentage(byte openingPercentage) {
        ...
        if ((openingPercentage >= CARDIO2E_MIN_CURTAIN_OPENING_PERCENTAGE)
            ...
            throw new IllegalArgumentException("invalid openingPercentage '" + openingPercentage + "'");
        }
    }
    ...
    public String toString() {
        ...
        if (isDataVerified()) {
            switch (getTransactionType()) {
                ...
                case SET:
                    returnString = String.format("%s%s %s %d %d%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(), openingPercentage,
                            CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
            }
        }
