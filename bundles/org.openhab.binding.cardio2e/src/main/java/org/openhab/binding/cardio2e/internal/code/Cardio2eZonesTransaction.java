...
public class Cardio2eZonesTransaction extends Cardio2eTransaction {
    ...
    public void setZoneStates(Cardio2eZoneStates zoneStates[]) {
        isDataComplete = false;
        if (zoneStates != null) {
            if ((zoneStates.length >= 1) && (zoneStates.length <= maxStatesLenght)) {
                this.zoneStates = zoneStates;
            } else {
                throw new IllegalArgumentException(
                        "invalid zoneStates length '" + zoneStates.length + "' or objectNumber not set");
            }
        } else {
    ...
    public Cardio2eZoneStates getZoneState() {
        ...
        if (zoneStates != null) {
            if ((getObjectNumber() >= CARDIO2E_MIN_OBJECT_NUMBER) && (zoneStates.length == 1)) {
    ...
    public boolean[] getZoneDetections() { // Returns boolean array with zones
        ...
        if ((objectNumber >= CARDIO2E_MIN_OBJECT_NUMBER) && (zoneStates != null) && (zonesMask != null)) {
            ...
            if (length > 0) {
                ...
                for (byte i = 0; i < length; i++) {
                    if (invertZoneDetection) {
                        zoneDetections[i] = (zoneStates[i] == zonesMask[i + zoneNumberOffset]);
                    } else {
                        zoneDetections[i] = (zoneStates[i] != zonesMask[i + zoneNumberOffset]);
                    }
                }
            ...
            throw new IllegalArgumentException("invalid data (check objectNumber / zoneStates / zonesMask)");
        }
    }
    ...
    public boolean getZoneDetection(short zoneNumber) { // Returns boolean value
        ...
        if ((zoneStates != null) && (zonesMask != null)) {
            ...
            if ((zoneNumber >= CARDIO2E_MIN_OBJECT_NUMBER)) {
                if ((zoneNumber >= objectNumber) && (zoneNumber < (objectNumber + zoneStates.length))) { // Checks
                                                                                                         // zone
                                                                                                         // range
                    zoneDetection = (zoneStates[(zoneNumber - objectNumber)] != zonesMask
                            .getZonesMask()[(zoneNumber - 1)]);
                    return (invertZoneDetection) ? !zoneDetection : zoneDetection;
                } else {
                    throw new IllegalArgumentException("zoneNumber '" + objectNumber + "' is out of range");
                }
            } else {
                throw new IllegalArgumentException("invalid objectNumber '" + objectNumber + "'");
            }
        } else {
            throw new IllegalArgumentException("invalid zoneStates or zonesMask");
        }
    }
