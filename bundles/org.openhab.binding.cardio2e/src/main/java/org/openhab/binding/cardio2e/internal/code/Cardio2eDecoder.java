...
public class Cardio2eDecoder {
    private static final Logger logger = LoggerFactory.getLogger(Cardio2eDecoder.class);
    ...
    private void decodeCardio2eTransaction(String transactionToDecode) {
        ...
        if (digestedTransaction.size() >= 2) { // Will process transactions with
            ...
            try {
                ...
                logger.warn("Illegal transaction type in decoding transaction: '{}'", ex.toString());
            }
            // Decode object type
            try {
                ...
                logger.warn("Illegal object type in decoding transaction: '{}'", ex.toString());
            }
            ...
            if ((transactionType != null) && (objectType != null)) {
                switch (objectType) {
                    case DATE_AND_TIME:
                        ...
                        decodeComplete = decodeDateTimeParameters(receivedCardio2eDateTimeTransaction,
                                digestedTransaction);
                    ...
                    case HVAC_CONTROL:
                        ...
                        decodeComplete = decodeHvacControlParameters(receivedCardio2eHvacControlTransaction,
                                digestedTransaction);
                    ...
                    case HVAC_TEMPERATURE:
                        ...
                        decodeComplete = decodeHvacTemperatureParameters(receivedCardio2eHvacTemperatureTransaction,
                                digestedTransaction);
                    ...
                    case LIGHTING:
                        ...
                        decodeComplete = decodeLightingParameters(receivedCardio2eLightingTransaction,
                                digestedTransaction);
                    ...
                    case SCENARIO:
                        ...
                        decodeComplete = decodeScenarioParameters(receivedCardio2eScenarioTransaction,
                                digestedTransaction);
                    ...
                    case SECURITY:
                        ...
                        decodeComplete = decodeSecurityParameters(receivedCardio2eSecurityTransaction,
                                digestedTransaction);
                    ...
                    case ZONES:
                        if (decodeZonesStateTransaction) {
                            ...
                            decodeComplete = decodeZonesParameters(receivedCardio2eZonesTransaction,
                                    digestedTransaction);
                    ...
                    case ZONES_BYPASS:
                        ...
                        decodeComplete = decodeZoneBypassParameters(receivedCardio2eZoneBypassTransaction,
                                digestedTransaction);
                    ...
                    case VERSION:
                        ...
                        decodeComplete = decodeVersionParameters(receivedCardio2eVersionTransaction,
                                digestedTransaction);
                    ...
                    case CURTAIN:
                        ...
                        decodeComplete = decodeCurtainParameters(receivedCardio2eCurtainTransaction,
                                digestedTransaction);
    ...
    private boolean decodeNACK(Cardio2eTransaction receivedCardio2eTransaction, List<String> digestedTransaction) {
        ...
        if (digestedTransaction.size() == 3) {
            try {
                ...
                logger.warn("Illegal argument in decoding NACK transaction: '{}'", ex.toString());
            }
        }
    ...
    private boolean decodeLightingParameters(Cardio2eLightingTransaction receivedCardio2eTransaction,
            ...
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding LIGHTING transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
    ...
    private boolean decodeRelayParameters(Cardio2eRelayTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding RELAY transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
                ...
                if (digestedTransaction.size() == 4) {
                    try {
                        if (digestedTransaction.get(3).length() == 1) {
                            receivedCardio2eTransaction.setObjectNumber(Short.parseShort(digestedTransaction.get(2)));
                            receivedCardio2eTransaction.setRelayState(
                                    Cardio2eRelayStates.fromSymbol(digestedTransaction.get(3).charAt(0)));
                            decodeComplete = true;
                        }
                    } catch (IllegalArgumentException ex) {
                        logger.warn("Illegal argument in decoding RELAY transaction: '{}'", ex.toString());
    ...
    private boolean decodeHvacTemperatureParameters(Cardio2eHvacTemperatureTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding HVAC_TEMPERATURE transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
                    ...
                    try {
                        if (digestedTransaction.get(4).length() == 1) {
                            ...
                            receivedCardio2eTransaction
                                    .setHvacTemperature(Double.parseDouble(digestedTransaction.get(3)));
                            receivedCardio2eTransaction.setHvacSystemMode(
                                    Cardio2eHvacSystemModes.fromSymbol(digestedTransaction.get(4).charAt(0)));
                            decodeComplete = true;
                        }
                    } catch (IllegalArgumentException ex) {
                        logger.warn("Illegal argument in decoding HVAC_TEMPERATURE transaction: '{}'", ex.toString());
                    } catch (Exception ex) {
                        logger.warn("Unespecified error in decoding HVAC_TEMPERATURE transaction: '{}'", ex.toString());
                    }
                }
                break;
        }
    ...
    private boolean decodeHvacControlParameters(Cardio2eHvacControlTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding HVAC_CONTROL transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
                ...
                if (digestedTransaction.size() == 7) {
                    try {
                        if ((digestedTransaction.get(5).length() == 1) && (digestedTransaction.get(6).length() == 1)) {
                            receivedCardio2eTransaction.setObjectNumber(Short.parseShort(digestedTransaction.get(2)));
                            receivedCardio2eTransaction
                                    .setHvacHeatingSetPoint(Double.parseDouble(digestedTransaction.get(3)));
                            receivedCardio2eTransaction
                                    .setHvacCoolingSetPoint(Double.parseDouble(digestedTransaction.get(4)));
                            receivedCardio2eTransaction.setHvacFanState(
                                    Cardio2eHvacFanStates.fromSymbol(digestedTransaction.get(5).charAt(0)));
                            receivedCardio2eTransaction.setHvacSystemMode(
                                    Cardio2eHvacSystemModes.fromSymbol(digestedTransaction.get(6).charAt(0)));
                            decodeComplete = true;
                        }
                    } catch (IllegalArgumentException ex) {
                        logger.warn("Illegal argument in decoding HVAC_CONTROL transaction: '{}'", ex.toString());
                    } catch (Exception ex) {
                        logger.warn("Unespecified error in decoding HVAC_CONTROL transaction: '{}'", ex.toString());
                    }
                }
                break;
        }
    ...
    @SuppressWarnings("incomplete-switch")
    private boolean decodeScenarioParameters(Cardio2eScenarioTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding SCENARIO transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
            ...
            case SET:
                ...
                if (digestedTransaction.size() == 4) {
                    try {
                        if (digestedTransaction.get(3)
                                .length() <= Cardio2eTransaction.CARDIO2E_MAX_SECURITY_CODE_LENGTH) {
                            receivedCardio2eTransaction.setObjectNumber(Short.parseShort(digestedTransaction.get(2)));
    ...
    @SuppressWarnings("incomplete-switch")
    private boolean decodeZonesParameters(Cardio2eZonesTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
            ...
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding ZONES transaction: '{}'", ex.toString());
                    }
                }
                break;
            case INFORMATION:
                if (digestedTransaction.size() == 4) {
                    try {
                        ...
                        if ((objectNumber >= Cardio2eTransaction.CARDIO2E_MIN_OBJECT_NUMBER)
                            ...
                            maxZonesToDecode = (Cardio2eTransaction.CARDIO2E_MAX_SECURITY_ZONE_NUMBER
                                    - (objectNumber - 1));
    ...
    private boolean decodeZoneBypassParameters(Cardio2eZonesBypassTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        short objectNumber = 0;
            ...
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding ZONES_BYPASS transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
            ...
            case INFORMATION:
                if (digestedTransaction.size() == 4) {
                    try {
                        ...
                        if ((objectNumber >= Cardio2eTransaction.CARDIO2E_MIN_OBJECT_NUMBER)
                                && (objectNumber <= Cardio2eTransaction.CARDIO2E_MAX_SECURITY_ZONE_NUMBER)) {
                            maxZonesBypassToDecode = (Cardio2eTransaction.CARDIO2E_MAX_SECURITY_ZONE_NUMBER
                                    - (objectNumber - 1));
                            parameters = digestedTransaction.get(3);
                            parametersLength = parameters.length();
    ...
    private boolean decodeDateTimeParameters(Cardio2eDateTimeTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
            ...
            case SET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding DATE_AND_TIME transaction: '{}'", ex.toString());
                    }
                }
                break;
        }
    ...
    private boolean decodeSecurityParameters(Cardio2eSecurityTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding SECURITY transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
                    ...
                    try {
                        ...
                        if ((receivedCardio2eTransaction.getObjectNumber() == 1)
                                && (digestedTransaction.get(3).length() == 1)) { // ObjectNumber
                                                                                 ...
                                                                                 // be
                                                                                 // 1
                            receivedCardio2eTransaction.setSecurityState(
                                    Cardio2eSecurityStates.fromSymbol(digestedTransaction.get(3).charAt(0)));
                            decodeComplete = true;
                        }
                    } catch (IllegalArgumentException ex) {
                        logger.warn("Illegal argument in decoding SECURITY transaction: '{}'", ex.toString());
    ...
    @SuppressWarnings("incomplete-switch")
    private boolean decodeLoginParameters(Cardio2eLoginTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
                if (digestedTransaction.size() == 2) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding LOGIN transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
            ...
            case SET:
                if (digestedTransaction.size() == 4) { // LOGIN command (or LOGOUT,
                                                       ...
                                                       // command)
                    try {
                        if ((digestedTransaction.get(2).length() == 1) && (digestedTransaction.get(3)
                                .length() <= Cardio2eTransaction.CARDIO2E_MAX_PROGRAM_CODE_LENGTH)) { // Login
                                                                                                      // command
                                                                                                      // length
                                                                                                      // must
                                                                                                      // be
                            ...
                            receivedCardio2eTransaction.setLoginCommand(
                                    Cardio2eLoginCommands.fromSymbol(digestedTransaction.get(2).charAt(0)));
    ...
    private boolean decodeCurtainParameters(Cardio2eCurtainTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
            case GET:
                if (digestedTransaction.size() == 3) {
                    try {
                        ...
                        logger.warn("Illegal argument in decoding CURTAIN transaction: '{}'", ex.toString());
                    }
                }
                break;
            case NACK:
    ...
    @SuppressWarnings("incomplete-switch")
    private boolean decodeVersionParameters(Cardio2eVersionTransaction receivedCardio2eTransaction,
            List<String> digestedTransaction) {
        boolean decodeComplete = false;
        switch (receivedCardio2eTransaction.getTransactionType()) {
            case ACK:
            case GET:
                if (digestedTransaction.size() == 3) {
                    if (digestedTransaction.get(2).length() == 1) {
                        try {
                            receivedCardio2eTransaction.setVersionType(
                                    Cardio2eVersionTypes.fromSymbol(digestedTransaction.get(2).charAt(0)));
                            decodeComplete = true;
                        } catch (IllegalArgumentException ex) {
                            logger.warn("Illegal argument in decoding VERSION transaction: '{}'", ex.toString());
                        }
                    }
                }
                break;
            ...
            case INFORMATION:
                if (digestedTransaction.size() == 4) {
                    try {
                        ...
                        logger.warn("Unespecified error in decoding VERSION transaction: '{}'", ex.toString());
                    }
                }
                break;
        }
