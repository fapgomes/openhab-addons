...
public class Cardio2eZonesBypassTransaction extends Cardio2eTransaction {
    ...
    public Cardio2eZonesBypassTransaction(short objectNumber, Cardio2eZoneBypassStates zoneBypassStates[]) { // Simple
                                                                                                             // SET
                                                                                                             // transactionType
                                                                                                             // constructor
                                                                                                             // for
    ...
    public Cardio2eZonesBypassTransaction(short objectNumber, boolean zoneBypassStates[]) { // Simple SET
                                                                                            // transactionType
                ...
                for (int i = 0; i < length; i++) {
                    if (zoneBypassStates[i] != null) {
                        if (zoneBypassStates[i] == Cardio2eZoneBypassStates.BYPASSED) {
                            ...
                            throw new IllegalArgumentException("invalid zoneBypassState value at position " + i);
                        }
                    } else {
                        throw new IllegalArgumentException("null zoneBypassState value at position " + i);
                    }
                }
            } else {
                throw new IllegalArgumentException(
                        "invalid zoneBypassStates length '" + zoneBypassStates.length + "' or objectNumber not set");
            }
        } else {
            throw new IllegalArgumentException("cannot parse null zoneBypassStates values");
        }
    ...
    public void setZoneBypassStates(Cardio2eZoneBypassStates zoneBypassStates[]) {
        isDataComplete = false;
        if (zoneBypassStates != null) {
            if ((zoneBypassStates.length >= 1) && (zoneBypassStates.length <= maxStatesLenght)) {
            ...
            throw new IllegalArgumentException("zoneBypassStates cannot be null");
        }
    }
    ...
    public void setZoneBypassBooleanStates(boolean zoneBypassStates[]) {
        ...
        if (zoneBypassStates != null) {
            ...
            if ((length >= 1) && (length <= maxStatesLenght)) {
                ...
                for (int i = 0; i < length; i++) {
                    boolean bypassState = (invertZoneBypassBooleanStates) ? !zoneBypassStates[i] : zoneBypassStates[i];
                    if (bypassState) {
    ...
    public Cardio2eZoneBypassStates getZoneBypassState() {
        ...
        if (zoneBypassStates != null) {
            if ((getObjectNumber() >= CARDIO2E_MIN_OBJECT_NUMBER) && (zoneBypassStates.length == 1)) {
    ...
    public boolean getZoneBypassBooleanState() {
        ...
        if (zoneBypassStates[0] != null) {
            if (zoneBypassStates[0] == Cardio2eZoneBypassStates.BYPASSED) {
                ...
                throw new IllegalArgumentException("invalid zoneBypassState value");
            }
        } else {
            throw new IllegalArgumentException("cannot parse null zoneBypassState value");
        }
    ...
    public void setZoneBypassState(Cardio2eZoneBypassStates zoneBypassState) {
        ...
        if (zoneBypassState != null) {
            if (getObjectNumber() >= CARDIO2E_MIN_OBJECT_NUMBER) {
                ...
                throw new IllegalArgumentException("cannot set zoneState because objectNumber is not properly defined");
            }
        } else {
    ...
    public String toString() {
        ...
        if (isDataVerified()) {
            switch (getTransactionType()) {
                case ACK:
                case GET:
                    returnString = String.format("%s%s %s %d%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(),
                            CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
                case NACK:
                    if (getObjectNumber() == -1) {
                        returnString = String.format("%s%s %s %d%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                                getTransactionType().symbol, getObjectType().symbol, getErrorCode(),
                                CARDIO2E_END_TRANSACTION_CHARACTER);
                    } else {
                        returnString = String.format("%s%s %s %d %d%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                                getTransactionType().symbol, getObjectType().symbol, getObjectNumber(), getErrorCode(),
                                CARDIO2E_END_TRANSACTION_CHARACTER);
                    }
                    break;
                case INFORMATION:
                case SET:
                    ...
                    returnString = String.format("%s%s %s %d %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(), returnString,
                            CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
            }
        }
