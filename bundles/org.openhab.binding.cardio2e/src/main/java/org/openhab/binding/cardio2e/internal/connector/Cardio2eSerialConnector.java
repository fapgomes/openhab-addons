...
public class Cardio2eSerialConnector {
    private static final Logger logger = LoggerFactory.getLogger(Cardio2eSerialConnector.class);
    ...
    protected void signalReceivedData(String data) {
        if (comEventListeners != null && !comEventListeners.isEmpty()) {
            Cardio2eReceivedDataEvent event = new Cardio2eReceivedDataEvent(this, data);
            Vector<Cardio2eSerialConnectorEventListener> targets; // make a copy of the
            // listener list in case
            // anyone adds/removes
            // listeners
            synchronized (this) {
                targets = (Vector<Cardio2eSerialConnectorEventListener>) comEventListeners.clone();
            }
            Enumeration<Cardio2eSerialConnectorEventListener> e = targets.elements();
            while (e.hasMoreElements()) {
                Cardio2eSerialConnectorEventListener l = (Cardio2eSerialConnectorEventListener) e.nextElement();
    ...
    protected void signalIsConnected(boolean isConnected) {
        if (comEventListeners != null && !comEventListeners.isEmpty()) {
            Cardio2eConnectionEvent event = new Cardio2eConnectionEvent(this, isConnected);
            Vector<Cardio2eSerialConnectorEventListener> targets; // make a copy of the
            // listener list in case
            // anyone adds/removes
            // listeners
    ...
    public void sendTransaction(Cardio2eTransaction transaction) { // Add
        ...
        if (transaction != null) {
            if (tryConnect()) {
                ...
                try {
                    Cardio2eTransaction sendingTransaction = transaction.deepClone(); // Prevents transaction modify
                                                                                      // after
                                                                                      // add to buffer
                    if (sendingTransaction.primitiveStringTransaction == null)
                        sendingTransaction.primitiveStringTransaction = sendingTransaction.toString();
                    if (sendingTransaction.isSmartSendingEnabled()) { // SMARTSENDING
                        ...
                        synchronized (sendBuffer) {
                            boolean matchesLastTransaction = primitiveStringMatchesLastTransactionLikeMeInSendBuffer(
                                    sendingTransaction, sendingTransaction.smartSendingCanReplaceLikeMe(),
                    ...
                    logger.warn("Unknown error in sending data: '{}'", ex.toString());
                } finally {
                    if (transactionAdded)
    ...
    private boolean primitiveStringMatchesLastTransactionLikeMeInSendBuffer(Cardio2eTransaction transaction,
            boolean purgeAllTransactionsLikeMe, boolean avoidPurgeLastTransactionIfPrimitiveStringMatches) {
        ...
        while (((!foundLastTransactionIsLike) || (purgeAllTransactionsLikeMe)) && (it.hasNext())) {
            currentTransaction = it.next();
            if ((currentTransaction.isLike(transaction))
                    && (currentTransaction.getTransactionType() == transaction.getTransactionType())) {
                if (foundLastTransactionIsLike) {
                    ...
                    if (currentTransaction.primitiveStringTransaction.equals(transaction.primitiveStringTransaction))
    ...
    public synchronized void disconnect() {
        if (isConnected()) {
            try {
                ...
                logger.warn("Unknown error in closing port: '{}'", ex.toString());
            }
            if (!isConnected()) {
                timer.cancel();
                timer.purge();
    ...
    private synchronized void tryLogout() throws InterruptedException {
        ...
        // compatibility.
        Cardio2eTransaction logoutTransaction = new Cardio2eLoginTransaction(Cardio2eLoginCommands.LOGOUT);
    ...
    public void setMinDelayBetweenReceivingAndSending(int minDelayBetweenReceivingAndSending) {
    ...
    private void calculateSendingTimerCyclePeriod() {
        ...
        sendingTimerCyclePeriod = ((a < MIN_SENDING_TIMER_CYCLE_PERIOD) ? MIN_SENDING_TIMER_CYCLE_PERIOD : a);
        logger.trace("Calculated Sending Timer Cycle Period is {} ms.", sendingTimerCyclePeriod);
    }
    ...
    private void processSend() {
        if (nextSendingTransaction != null) { // Clear nextSendingTransaction if
            ...
            primitiveSentTransaction = primitiveSentTransaction.substring(0, primitiveSentTransaction.length() - 1); // Removes
                                                                                                                     // end
                                                                                                                     // transaction
                                                                                                                     // char
            if ((nextSendingTransaction.receiptACK) || (nextSendingTransaction.sendingTries >= MAX_SENDING_TRIES)) {
                try {
                    sendBuffer.remove(nextSendingTransaction);
                } catch (Exception ex) {
                    logger.warn("Error in removing transaction from sending buffer: '{}'", ex.toString());
                } finally {
                ...
                logger.debug("No ACK was received for '{}'. Will send it again...", primitiveSentTransaction);
            }
        }
        ...
        if (nextSendingTransaction == null) {
            ...
            try {
                ...
                if (testMode) { // Adapts END transaction
                    ...
                    stringTransaction = stringTransaction.replace(
                            Cardio2eTransaction.CARDIO2E_END_TRANSACTION_CHARACTER,
                            Cardio2eSerialConnector.CARDIO2E_END_TRANSACTION_TEST_CHARACTER);
                }
                serialPort.writeString(stringTransaction);
                stringTransaction = stringTransaction.substring(0, stringTransaction.length() - 1); // Removes
                                                                                                    // end
                                                                                                    // transaction
                                                                                                    // char
                logger.debug("Sent '{}' to Cardio 2é", stringTransaction);
            } catch (SerialPortException ex) {
                logger.warn("There was an error on writing string to port: '{}'", ex.toString());
            } catch (Exception ex) {
                logger.warn("Unexpected error in sending data: '{}'", ex.toString());
            } finally {
                try {
                    if ((nextSendingTransaction.getTransactionType() != Cardio2eTransactionTypes.SET)
                            || (nextSendingTransaction.getObjectType() == Cardio2eObjectTypes.LOGIN) || (testMode))
                    ...
                    logger.warn("Unexpected error in processing send data: '{}'", ex.toString());
                }
            }
        }
    }
            ...
            if (event.isRXCHAR() && event.getEventValue() > 0) {
                try {
                    ...
                    if (testMode) { // Adapts END transaction
                                    // character to test console
                                    // when test mode is enabled
                        data = data.replace(Cardio2eSerialConnector.CARDIO2E_END_TRANSACTION_TEST_CHARACTER,
                                Cardio2eTransaction.CARDIO2E_END_TRANSACTION_CHARACTER);
                    }
                    ...
                    logger.warn("Error in receiving string from COM-port: '{}'", ex.toString());
                } catch (Exception ex) {
                    logger.warn("Error in processing string from COM-port: '{}'", ex.toString());
                }
            }
        }
    }
    ...
    private class CyclicSend extends TimerTask {
        public synchronized void run() {
            if ((lastReceivedTimeStamp + minDelayBetweenReceivingAndSending) <= (System.currentTimeMillis())) {
                if ((lastSendTimeStamp + minDelayBetweenSendings) <= (System.currentTimeMillis())) {
                    processSend();
                }
            }
        }
    ...
    private class DecodedTransactionListener implements Cardio2eDecodedTransactionListener {
        ...
        public void decodedTransaction(Cardio2eDecodedTransactionEvent e) {
            ...
            primitiveReceivedTransaction = primitiveReceivedTransaction.substring(0,
                    primitiveReceivedTransaction.length() - 1); // Removes
                                                                // end
                                                                // transaction
                                                                // char
                            ...
                            if (lastSentTransaction.isLike(transaction)) {
                                ...
                                primitiveSentTransaction = primitiveSentTransaction.substring(0,
                                        primitiveSentTransaction.length() - 1); // Removes
                                                                                ...
                                                                                // transaction
                                                                                // char
                                logger.debug("Receipt ACK: '{}' {} transaction RECEIVED.", primitiveSentTransaction,
                                        lastSentTransaction.getObjectType());
                            }
                        }
                    }
                } catch (Exception ex) {
                    logger.warn("Unexpected error in checking sent transaction receipt ACK: '{}'", ex.toString());
                }
            }
        }
    }
