...
public class Cardio2eLoginTransaction extends Cardio2eTransaction {
    ...
    public void setTransactionType(Cardio2eTransactionTypes transactionType) {
        if (transactionType != Cardio2eTransactionTypes.GET) {
            ...
            throw new IllegalArgumentException(
                    "invalid transactionType '" + transactionType + "for a LOGIN transaction");
        }
    }
    ...
    public void setProgramCode(String programCode) {
        isDataComplete = false;
        if ((programCode.length() <= CARDIO2E_MAX_PROGRAM_CODE_LENGTH) && StringUtils.isNumeric(programCode)) {
            this.programCode = programCode;
        } else {
            throw new IllegalArgumentException("invalid programCode '" + programCode + "'");
        }
    }
        ...
        if (super.isDataVerified()) {
            switch (getTransactionType()) {
                ...
                case NACK:
                    ...
                    break;
                case SET:
                    verified = ((loginCommand == Cardio2eLoginCommands.LOGOUT)
                            || ((loginCommand != null) && (programCode != null)));
                    break;
            }
        }
        return verified;
    ...
    public String toString() {
        ...
        if (isDataVerified()) {
            switch (getTransactionType()) {
                case ACK:
                    returnString = String.format("%s%s %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
                case NACK:
                ...
                case INFORMATION: // Signals Login Information End (@I P E)
                    returnString = String.format("%s%s %s E%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
                case SET:
                    if (programCode != null) {
                        returnString = String.format("%s%s %s %s %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                                getTransactionType().symbol, getObjectType().symbol, loginCommand.symbol, programCode,
                                CARDIO2E_END_TRANSACTION_CHARACTER);
                    } else {
                        returnString = String.format("%s%s %s %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                                getTransactionType().symbol, getObjectType().symbol, loginCommand.symbol,
                                CARDIO2E_END_TRANSACTION_CHARACTER);
                    }
                    break;
            }
