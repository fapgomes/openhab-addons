...
public class Cardio2eSecurityTransaction extends Cardio2eTransaction {
    ...
    public Cardio2eSecurityTransaction(Cardio2eSecurityStates securityState, String securityCode) { // Simple SET
                                                                                                    // security arm /
                                                                                                    // disarm
                                                                                                    // transaction
                                                                                                    // constructor.
    ...
    public void setSecurityCode(String securityCode) {
        isDataComplete = false;
        if ((securityCode.length() <= CARDIO2E_MAX_SECURITY_CODE_LENGTH) && StringUtils.isNumeric(securityCode)) {
            this.securityCode = securityCode;
        } else {
            throw new IllegalArgumentException("invalid securityCode '" + securityCode + "'");
        }
    }
            ...
            switch (getTransactionType()) {
                ...
                case INFORMATION:
                    returnString = String.format("%s%s %s %d %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(),
                            securityState.symbol, CARDIO2E_END_TRANSACTION_CHARACTER);
                case SET:
                    returnString = String.format("%s%s %s %d %s %s%s", CARDIO2E_START_TRANSACTION_INITIATOR,
                            getTransactionType().symbol, getObjectType().symbol, getObjectNumber(),
                            securityState.symbol, securityCode, CARDIO2E_END_TRANSACTION_CHARACTER);
                    break;
            }
        }
        return returnString;
